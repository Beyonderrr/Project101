...
void bullet_item2_fly()
{
	for (int i = 0; i < countofbullet; i++)
	{
		...
		{
			if (bullet_item21[i].Y < 0 || bullet_item21[i].X < 1)
			{
	...
	for (int i = 0; i < countofbullet; i++)
	{
		...
		{
			if (bullet_item23[i].Y < 0 || bullet_item23[i].X > 88)
			{
	...
	for (int i = 0; i < countofbullet; i++)
	{
		...
		{
			if (bullet_item24[i].Y < 0 || bullet_item24[i].X < 1)
			{
	...
	for (int i = 0; i < countofbullet; i++)
	{
		...
		{
			if (bullet_item25[i].Y < 0 || bullet_item25[i].X > 88)
			{
...
void fill_Score_to_buffer()
{
	...
	for (int j = 0; j < 9; j++)
	{
		consoleBuffer[106 - j + screen_x * 6].Char.AsciiChar = 48 + score[j];
		consoleBuffer[106 - j + screen_x * 6].Attributes = 7;
	}
	for (int k = 0; k < 9; k++)
	{
		score[k] = 0;
